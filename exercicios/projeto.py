...
df['Preco'] = df['Preco'].fillna('R$ 0.00')
...
df['Quantidade_Vendida'] = df['Quantidade_Vendida'].fillna(1)
...
df_faturamento_autor = df.groupby('Autor')['Faturamento'].sum()
print("\nFaturamento Total por Autor")
print(df_faturamento_autor)

# b) Qual foi o faturamento total por **`Genero`**?

df_faturamento_genero = df.groupby('Genero')['Faturamento'].sum()
print("\nFaturamento Total por Genero")
print(df_faturamento_genero)

# c) Qual livro foi o mais vendido em **quantidade**?

df_mais_vendido = df.groupby('Livro')['Quantidade_Vendida'].sum()
df_mais_vendido = df_mais_vendido.sort_values(ascending = False)
print("\nLivros Mais Vendidos")
print(df_mais_vendido)
...
df_faturamento_mes = df.resample('ME')['Faturamento'].sum()
